...
# fantrax_prev_df = pd.read_csv(f"fantrax_prices_AFTER_draft.csv")
...
fantrax_prev_rostered = dict(zip(fantrax_prev_df['name_mapped'], fantrax_prev_df['isPartOfRoster']))
...
def compute_new_price(row):
    # If no team (meaning the fantrax data does not include an NBA team assigned to this player i.e Calathes) → return 1
    ...
    # Check if is rostered
    is_rostered = bool(fantrax_prev_rostered.get(row['name_norm'], False))

    try:
        if is_rostered:
            new_price = round(1.1 * ((0.62 * float(fantrax_price)) + (0.38 * float(yahoo_price))))
        else:
          # new_price = int(yahoo_price) #before draft
            new_price = 5 #after draft
    except:
        new_price = 1
...
# ----------------- SAVE RESULT -----------------
output_file = f"final_prices_{current_year}_before_draft.csv"
# output_file = f"final_prices_{current_year}_after_draft.csv"
